...
import com.oldmee.nutz.util.Toolkit;
...
/**
 * @Author: R.oldmee
 * @Description:UserProfile增删改查方法
 * @Date: Create in 13:39 2019/1/21
 */
@IocBean
@At("/user/profile")
public class UserProfileModule extends BaseModule {
    ...
    public Object activeMail(@Attr(scope = Scope.SESSION, value = "me") int userId,
        ...
        token = com.oldmee.nutz.util.Toolkit._3DES_encode(emailKEY, token.getBytes());
    ...
    public String activeMailCallback(@Param("token") String token, HttpSession session) {
        ...
        try {
            token = com.oldmee.nutz.util.Toolkit._3DES_decode(emailKEY, Toolkit.hexstr2bytearray(token));
            if (token == null)
                return "非法token";
